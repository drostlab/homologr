#' @title Perform a DIAMOND best reciprocal hit search
...
#' rec_best_hits
#'
#' # run diamond_reciprocal_best_hits() with multiple cores
#' rec_best_hits <- diamond_reciprocal_best_hits(
#' query   = system.file('seqs/ortho_thal_aa.fasta', package = 'homologr'),
#' subject = system.file('seqs/ortho_lyra_aa.fasta', package = 'homologr'),
#' cores   = 2)
...
#' }
#'
#' @return A tibble as returned by the \code{diamond_reciprocal_best_hits} function, storing the \code{query_ids}
#' in the first column and the \code{subject_ids} (reciprocal best hit homologs) in the second column.
#' @seealso \code{\link{diamond_best_hits}}, \code{\link[rdiamond]{diamond_protein_to_protein}}
#' @export

diamond_reciprocal_best_hits <- function(
        query,
        subject,
        is_subject_db = FALSE,
        format          = "fasta",
